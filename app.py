...
def generate_old_files_section():
    ...
    links_html = "\n".join([
        f'<li><a href="{OLD_DIR}/{fname}" target="_blank" class="text-blue-600 hover:underline">{fname}</a></li>'
    ...
    <div class="mt-10 bg-white p-4 rounded-lg shadow">
        ...
        <div>
              <ul class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 list-disc list-inside gap-2">
                {links_html}
              </ul>
        </div>
    </div>
